...
input_size = 784
num_classes = 10
embed_usps_train_X = embed_usps_X[:train_num]
embed_usps_train_Y = embed_usps_Y[:train_num]
embed_usps_test_X = embed_usps_X[train_num:train_num + test_num]
embed_usps_test_Y = embed_usps_Y[train_num:train_num + test_num]
best_acc = 0.0

for k in range(1, 30, 2):
    for epochs in range(5, 40, 2):
        embed_knn_sv, *_ = old_knn_shapley(k, embed_usps_train_X, embed_usps_test_X, embed_usps_train_Y, embed_usps_test_Y)
        ...
        x_train = torch.from_numpy(raw_usps_X[filted_embed_knn_sv_idxs]).contiguous().view(-1, 1, 28, 28)
        y_train = torch.from_numpy(raw_usps_Y[filted_embed_knn_sv_idxs]).view(-1,).long()

        x_test = torch.from_numpy(raw_mnist_X[:val_num]).contiguous().view(-1, 1, 28, 28)
        y_test = torch.from_numpy(raw_mnist_Y[:val_num]).view(-1,).long()
